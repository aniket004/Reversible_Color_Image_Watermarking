function [ ybr_wm peak_y_value peak_cb_value peak_cr_value embedded_bits ] = ybrHistogram( ybr, b )
...
peak_cb_value = peak_pos_cb - 1; %grayscale having peak (max) no. of pixels
...
peak_cr_value = peak_pos_cr - 1; %grayscale having peak (max) no. of pixels
    ...
    for j = 1:nc
        if( b_pos <= lenb )
            if( coeff(i,j,3) == peak_cr_value )
                ...
                b_pos = b_pos + 1;
            elseif( coeff(i,j,3) > peak_cr_value )
    ...
    for j = 1:nc
        if( b_pos <= lenb )
            if( coeff(i,j,2) == peak_cb_value )
                ...
                b_pos = b_pos + 1;
            elseif( coeff(i,j,2) > peak_cb_value )
...
ybr_wm = sgn.*( im_wm + details );

ybr_rgb_wm = ybr2rgb( ybr_wm );

%figure
%imsave( imshow( uint8( ybr_rgb_wm ) ) );
